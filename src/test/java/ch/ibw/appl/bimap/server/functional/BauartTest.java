...
//    assertEquals(HttpStatus.NOT_ACCEPTABLE_406, response.code());
  ...
  public void get_byId() {
    HttpResponse response = executeGet("/bauarten/1");
    ...
    Bauart bauarten = new JSONSerializer().deserialize(body, new TypeReference<Bauart>() {});
    assertEquals(Bauart.create(1, "Einfamilienhaus").bauart, bauarten.bauart);
  }

  @Test
  public void get_byId_notFound() {
    HttpResponse response = executeGet("/bauarten/13");
    String body = new String(response.body());
