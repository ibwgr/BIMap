...
import {Leistung} from "./leistung.js";
...
export async function getAllBauherrenData() {
    let leistungen = [];
    let response = await fetch("http://localhost:2567/leistungen");
    let data = await response.json();
    for (let item of data) {
        //console.log(item)
        let leistung = new Leistung(item)
        leistungen.push(leistung.leistungen)
    }
    return leistungen;
}

export async function getAllBauartenData() {
    let bauarten = [];
    let response = await fetch("http://localhost:2567/bauarten");
    let data = await response.json();
    for (let item of data) {
        console.log(item)
        let bauart = new Bauart(item)
        bauarten.push(bauart.bauart)
    }
    console.log(bauarten)
    return bauarten;
}

export async function getAllProjektData() {
    let projects = [];
    let response = await fetch("http://localhost:2567/projekte", {
...
export async function getProjektDataById(projektId) {
    let project;
    let ReqURL = "http://localhost:2567/projekte/" + projektId;
    let response = await fetch(ReqURL)
    let data = await response.json();
    project = new Projekt(data)
