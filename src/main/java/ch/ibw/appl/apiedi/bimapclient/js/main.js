...
let markers = Controller.getTestKoordId()
for (let item of markers) {
    createVectorLayer([item.koordx, item.koordy])
    createPopups([item.koordx, item.koordy])
}
// Create the layer with the icon
function createVectorLayer(position) {
...
// Create the popup
function createPopups(position, idprojekt) {
    new OpenLayer
    var popup = new ol.Overlay({
        position: position,
        element: $(
            '<div class="popup">' +
            '<div class="title"<h3>Projekt soundso</h3></div>' +
            '<div class="subtitle">' +
